...
const Theme = () => {
  ...
  const formHandler = async (e) => {
    e.preventDefault();
    const form = e.target;
    const themeValue = form.theme.value;
    const response = await theme.create({ name: themeValue });
    if (response.status === 200) {
      theme.read().then((res) => setAllThemes(res));
      form.reset();
  ...
  const handleDelete = async (id) => {
    const res = await theme.delete(id);
    if (res.status === 200) {
      theme.read().then((res) => setAllThemes(res));
